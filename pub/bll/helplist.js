...
const roles = {
  ...
  async getList(ctx) {
    ...
    ctx.request.body.wheres =
      " helplist.openid = wxuser.openid and helplist.is_delete = 0  and helplist.title in ('打印服务','快递代取','校园跑腿','上门维修','代替服务','其他帮助') and helplist.a_id=" +
      ctx.request.body.a_id;
    // ctx.request.body.wheres =
    //   " helplist.wx_id = wxuser.id and helplist.is_delete = 0 and helplist.state in (1,2,3) and helplist.title in ('打印服务','快递代取','校园跑腿','上门维修','代替服务','其他帮助') and helplist.a_id=" +
    //   ctx.request.body.a_id;
    ctx.request.body.sorts = " helplist.create_time desc";
    let result = await com.commonSelect.getList(ctx);
    console.log("cccccccccccccccccccccc", result);
    if (result.args) {
      let userResult = await model.getList(result.args, result.ct);
      console.log("ddddddddddddddddddddddddd", userResult);
